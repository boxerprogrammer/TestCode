...
public class MaterialExporter
{
    ...
    static void RecursiveWriteData(BinaryWriter bw, GameObject obj)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                //テクスチャ名
                var texture = meshRen.material.mainTexture;
                ...
                var n = texpath.LastIndexOf("/");
                var texfileName = texpath.Substring(n + 1);

                //メッシュ名
                bw.Write(mesh.sharedMesh.name);
                //テクスチャ名
                bw.Write(texfileName);
    ...
    [MenuItem("情報出力/テクスチャ情報出力")]
    static void OutputLocationData()
    {
        var path = EditorUtility.SaveFilePanel("テクスチャ情報出力",
                                    ".",
                                    "texturedata.dat",
                                    "dat");
        if(path == "")
